data {
  ...
  int<lower = 0, upper = 108> z_obs[z_N_obs]; // Non-missing Sassad obs
  real<lower = 0, upper = 1> Treat[N]; // Daily treatment usage
  
  int<lower = 2, upper = 10> M; // Upper bound of Bother
  int<lower = 2, upper = 108> D; // Upper bound of Sassad
}
...
model {
  ...
  z_c0 ~ normal(0.07, 0.015);
  z_delta ~ normal(0.14, 0.07);
  ...
  yc_obs ~ ordered_logistic(y_lat[idx_obs], y_ct);
  zc_obs ~ ordered_logistic(y_lat[z_idx_obs], z_ct);
}
